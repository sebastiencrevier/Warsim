...
namespace Warsim.Core.Helpers.OAuth
{
    ...
    public class Manager
    {
        ...
        private string GenerateNonce()
        {
            ...
            for (int i=0; i < 8; i++)
            {
                int g = this._random.Next(3);
                switch(g)
                {
                    case 0:
                        // lowercase alpha
                        sb.Append( (char)(this._random.Next(26)+97), 1);
                        break;
                    default:
                        // numeric digits
                        sb.Append( (char)(this._random.Next(10)+48), 1);
                        break;
                }
            }
            return sb.ToString();
        ...
        {
            return this.AcquireRequestToken("https://api.twitter.com/oauth/request_token",
                                       "POST");
        }
        ...
        public OAuthResponse AcquireAccessToken(string uri, string method, string pin)
        {
            ...
            try
            {
                ...
                var buffer = Encoding.ASCII.GetBytes($"oauth_verifier={pin}");
                ...
                using (var response = (System.Net.HttpWebResponse)request.GetResponse())
                {
                    using (var reader = new System.IO.StreamReader(response.GetResponseStream()))
                    {
                        ...
                        this["token"] = r["oauth_token"];
                        this["token_secret"] = r["oauth_token_secret"];
                        this._params["username"] = r["screen_name"];
                        this._params["userid"] = r["user_id"];
        ...
        {
            return this.AcquireAccessToken("https://api.twitter.com/oauth/access_token",
                                      "POST",
                                      pin);
        }
